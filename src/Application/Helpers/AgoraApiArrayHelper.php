...
namespace AgoraApi\Application\Helpers;

class AgoraApiArrayHelper
{
    private $max_scan_depth = 30;
     ...
     * @return Setting
     */
    public function collection_get(\Slim\Collection $collection, string $key)
    {
        foreach (explode('.', $key) as $segment) {
    ...
    public function str_replace_deep($search, $replace, $collection)
    {
        if (is_array($collection))
        {
            foreach($collection as &$oneSubject)
                $oneSubject = $this->str_replace_deep($search, $replace, $oneSubject);
            unset($oneSubject);
            return $collection;
        } else {
            return str_replace($search, $replace, $collection);
        }
    }
     ...
     */
    public function require_all($dir, $app, $depth = 0) {
        ...
        foreach ($scan as $path) {
            ...
            elseif (is_dir($path)) {
                $this->require_all($path, $app, $depth + 1);
            }
        }
    }
     ...
     * @param int     $depth (optional)
     */
    public function add_definitions_all($dir, $builder, $depth = 0) {
        if ($depth > $this->max_scan_depth) {
            return;
        }
        // require all php files
        ...
        foreach ($scan as $path) {
            ...
            elseif (is_dir($path)) {
                $this->add_definitions_all($path, $builder, $depth + 1);
            }
        }
    }    
}
